...
def raw_data_rand_modify(raw_data, item_num, max_seqs_len, p):
    """
    ...
    l1_ground_truth = torch.zeros([max_seqs_len], dtype=torch.long)
    ...
    while raw_index < len(raw_data):
        ...
        if decision <= p:  # keep
            ...
            random_item = random.sample(available_item_pool, 1)
            random_modified_data.append(random_item[0])
            available_item_pool.remove(random_item[0])
            l1_ground_truth[raw_index] = 1
            raw_index += 1

    random_modified_data = random_modified_data + [0] * (max_seqs_len - len(random_modified_data))
    random_modified_data = torch.tensor(random_modified_data, dtype=torch.long)

    return random_modified_data, l1_ground_truth


class TrainDataset(Data.Dataset):
    def __init__(self, dir_path, 
                 item_num, max_seqs_len, 
                 max_insert_num, mask_prob, p):
    ...
    def __getitem__(self, index):
        ...
        padding_mask = torch.zeros([self.max_seqs_len], dtype=torch.float)
        padding_mask[:len(seq)] = 1

        random_modified_seqs, l1_ground_truth = raw_data_rand_modify(seq, 
                                                                     self.item_num, 
                                                                     self.max_seqs_len, 
                                                                     self.p)

        rec_loss_mask = torch.zeros([self.max_seqs_len], dtype=torch.float)
        rec_loss_mask[np.random.rand(self.max_seqs_len) < self.mask_prob] = 1
        rec_loss_mask *= padding_mask

        masked_seq = seq.copy()
        masked_seq = masked_seq + [0] * (self.max_seqs_len - len(masked_seq))
        masked_seq = torch.tensor(masked_seq).long()
        masked_seq[rec_loss_mask == 1] = self.item_num - 1

        seq = seq + [0] * (self.max_seqs_len - len(seq))
        seq = torch.tensor(seq).long()


        return seq, masked_seq, random_modified_seqs, l1_ground_truth, rec_loss_mask
...
class ValidDataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq = seq[-self.modified_max_seqs_len:]
        masked_seq = seq.copy()
